package kb_communitymodeling::kb_communitymodelingClient;
...
kb_communitymodeling::kb_communitymodelingClient
...
sub new
{
    ...
    my $self = {
	client => kb_communitymodeling::kb_communitymodelingClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
# Authentication: required
	...
	Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
							       "Invalid argument count for function predict_trophic_interactions (received $n, expecting 1)");
    }
    {
	my($params) = @args;
    ...
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "kb_communitymodeling.predict_trophic_interactions",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method predict_trophic_interactions",
...
# Authentication: required
    ...
    {
	Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
							       "Invalid argument count for function meta_genome_model_construction (received $n, expecting 1)");
    }
    {
	my($params) = @args;
    ...
    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "kb_communitymodeling.meta_genome_model_construction",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method meta_genome_model_construction",
					    status_line => $self->{client}->status_line,
					    method_name => 'meta_genome_model_construction',
				       );
    }
...
sub status
{
    ...
    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
        method => "kb_communitymodeling.status",
        params => \@args,
    });
    if ($result) {
        if ($result->is_error) {
...
sub version {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "kb_communitymodeling.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(
            error => "Error invoking method meta_genome_model_construction",
            status_line => $self->{client}->status_line,
            method_name => 'meta_genome_model_construction',
        );
    }
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for kb_communitymodeling::kb_communitymodelingClient\n";
    }
    if ($sMajor == 0) {
        warn "kb_communitymodeling::kb_communitymodelingClient version is $svr_version. API subject to change.\n";
    }
}
...
package kb_communitymodeling::kb_communitymodelingClient::RpcClient;
